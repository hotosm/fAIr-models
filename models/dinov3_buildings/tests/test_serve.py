...
from models.dinov3_buildings.pipeline import predict
...
def test_predict_returns_feature_collection(toy_chips: Path) -> None:
    result = predict(_StubSession(), str(toy_chips), {"confidence_threshold": 0.5})
...
def test_predict_requires_confidence_threshold(toy_chips: Path) -> None:
    with pytest.raises(ValueError, match="confidence_threshold"):
        predict(_StubSession(), str(toy_chips), {})
