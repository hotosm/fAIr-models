"""Step tests for DINOv3 buildings.
...
def pretrained_weights(tmp_path_factory: pytest.TempPathFactory) -> str:
    """Use the same Lightning checkpoint the platform passes as `base_model_weights`."""
    from huggingface_hub import hf_hub_download

    return hf_hub_download(
        repo_id="kshitijrajsharma/dinov3-hot-buildings",
        filename="dinov3l_upernet_hot.ckpt",
    )


def test_split_dataset(toy_chips: Path, toy_labels: Path, base_hyperparameters: dict[str, Any]) -> None:
    from models.dinov3_buildings.pipeline import split_dataset
...
def test_train_model(
    ...
    from models.dinov3_buildings.pipeline import split_dataset, train_model
    ...
    base_hyperparameters = {**base_hyperparameters, "epochs": 1}
...
def test_evaluate_model(
    ...
    from models.dinov3_buildings.pipeline import evaluate_model, split_dataset, train_model
...
def test_export_onnx(base_hyperparameters: dict[str, Any]) -> None:
    ...
    from models.dinov3_buildings.pipeline import export_onnx

    model = build_model(load_config(None)).cpu().net
