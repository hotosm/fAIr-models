"""Per-model test fixtures: toy OAM-tiled RAMP dataset (chips + labels).

``create_toy_data`` is importable by ``models/test_integration.py`` to materialize a
minimal 4-class semantic-segmentation dataset. Chip TIFFs follow the OAM naming
convention ``OAM-{x}-{y}-{z}.tif`` so ``hot_fair_utilities.preprocessing.clip_labels``
can recover the tile bounds.
"""
...
def create_toy_data(root: Path) -> dict[str, Path]:
    ...
    for i in range(CHIP_COUNT):
        ...
        # Tiny square polygon well inside each chip footprint.
    ...
    # `models/ramp/pipeline.py` expects `dataset_labels` to be a single GeoJSON file.
...
def generate_toy_dataset(tmp_path_factory: pytest.TempPathFactory) -> dict[str, Path]:
    return create_toy_data(tmp_path_factory.mktemp("toy_ramp"))
...
def _build_dataset_stac_item(chips_dir: Path, labels_geojson: Path) -> dict[str, Any]:
    return {
        ...
        "properties": {
            ...
            "description": "Toy RAMP semantic-segmentation dataset",
            "label:type": "vector",
