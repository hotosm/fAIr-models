...
def create_toy_data(root: Path) -> dict[str, Path]:
    ...
    with open(labels_path, "w", newline="", encoding="utf-8") as f:
        ...
        writer.writeheader()
        for i, chip in enumerate(sorted(chips_dir.glob("*.tif"))):
            writer.writerow({"filename": chip.name, "class_name": "building" if i % 2 == 0 else "no_building"})
...
def _build_dataset_stac_item(chips_dir: Path, labels_path: Path) -> dict[str, Any]:
    return {
        ...
        "properties": {
            ...
            "label:description": "Classification labels",
            "keywords": ["building"],
            "fair:user_id": "test",
            "version": "1",
            "deprecated": False,
