...
CHIP_COUNT = 6
CHIP_SIZE = 256
BASE_LON, BASE_LAT, STEP = 115.25, -8.71, 0.001
...
def generate_toy_dataset(tmp_path_factory: pytest.TempPathFactory) -> dict[str, Path]:
    return create_toy_data(tmp_path_factory.mktemp("toy_swimming_pool_detection"))
...
def _build_dataset_stac_item(chips_dir: Path, labels_dir: Path) -> dict[str, Any]:
    return {
        ...
        "properties": {
            ...
            "label:classes": [{"name": "swimming_pool", "classes": ["swimming_pool"]}],
            "label:description": "Detection labels",
            "keywords": ["swimming_pool"],
            "fair:user_id": "test",
            "version": "1",
            "deprecated": False,
            "license": "CC-BY-4.0",
