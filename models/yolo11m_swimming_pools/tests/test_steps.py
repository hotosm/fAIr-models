"""Step tests for YOLOv11m swimming pool detection.

Each test runs the real @step entrypoint against toy chips (256x256) and a
GeoJSON labels directory; YOLO's trainer handles internal resize to imgsz.
No pipeline-internal mocks. Telemetry sinks (zenml/mlflow) are no-ops via
...
def pretrained_weights(tmp_path_factory: pytest.TempPathFactory) -> str:
    ...
    cache = tmp_path_factory.mktemp("yolo_weights") / "model.pt"
...
def test_split_dataset_accepts_coco_json_labels(
    ...
    coco_path.write_text(
        json.dumps({"images": images, "annotations": annotations, "categories": [{"id": 0, "name": "swimming_pool"}]})
    )
