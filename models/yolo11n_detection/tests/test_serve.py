"""Serving-path tests for YOLO11n detection predict()."""
...
def fake_session() -> Any:
    ...
    session.run.return_value = [np.zeros((1, 5, 8400), dtype=np.float32)]
    return session


def test_predict_returns_feature_collection(fake_session: Any, tmp_path: Path) -> None:
    from models.yolo11n_detection.pipeline import predict

    _write_dummy_geotiff(tmp_path / "chip1.tif")
    result = predict(fake_session, str(tmp_path), {"confidence_threshold": 0.25})
    assert result["type"] == "FeatureCollection"
    assert isinstance(result["features"], list)


def test_predict_raises_without_confidence_threshold(fake_session: Any, tmp_path: Path) -> None:
    from models.yolo11n_detection.pipeline import predict
