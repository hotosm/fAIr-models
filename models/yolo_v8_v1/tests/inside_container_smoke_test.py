"""End-to-end smoke tests for models/yolo_v8_v1 Docker runtime.
...
Data layouts supported:
  - data/sample: --dataset-root /workspace/data/sample
    Uses train/oam + train/osm + predict/oam. Converts to YOLO input layout.
  - Legacy: --dataset-root /path/to/dataset
    Expects dataset/input/*.png + labels.geojson, dataset/prediction/input/.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run YOLO v8 v1 container smoke tests.")
    parser.add_argument(
        "--dataset-root",
        default="/workspace/data/sample",
        help="Dataset root: data/sample (train/oam + train/osm + predict/oam) or legacy layout.",
    )
    parser.add_argument(
        "--weights",
        default="yolov8s_v1-seg-best.pt",
...
def _prepare_data_sample_layout(dataset_root: Path) -> Path:
    ...
    Creates yolo_work/input/ (PNG + labels.geojson from train/) and
    yolo_work/prediction/input/ (from predict/oam).
    """
    ...
    for tif in pred_tifs[:5]:  # Limit for smoke test
        shutil.copy(tif, pred_input_dir / tif.name)

    print(f"PASS: prepared {len(tif_files)} train PNG + labels, {len(pred_tifs)} predict chips from data/sample")
    return work_root
...
def main() -> None:
    ...
    from hot_fair_utilities.training.yolo_v8_v1.train import train
